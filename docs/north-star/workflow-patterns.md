...
```
1. xray (Explore)    → What's in this codebase?
2. xray (Find)       → Where is authentication?
...
3. xray              → Understand the whole cluster
```
...
```
1. xray (error text) → Find where error originates
...
```
1. xray (Understand) → How does this work today?
2. query (edges)     → Find all affected locations
3. xray (Find)       → Find similar code as pattern
...
2. xray (explore)     → See unified view across all sources
