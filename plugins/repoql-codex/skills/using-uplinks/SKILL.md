...
- **Know what survives replacement.** A startup snapshot is not continuous backup. Later admin imports need a seed entry or a later offline snapshot; uncommitted/local-only changes need capture before losing the working volume. Never archive a live workspace or write the archive inside it. Existing EFS workspaces need the documented migration before switching layouts.
