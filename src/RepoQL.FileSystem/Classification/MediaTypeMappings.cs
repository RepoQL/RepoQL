...
internal static class MediaTypeMappings
{
    ...
    internal static bool TryGetMappedMediaType(IFileInfo fileInfo, out SemanticMediaType mediaType)
    {
        ...
        foreach (var (suffix, mapped) in CompoundExtensions)
        {
            ...
            {
                mediaType = SemanticMediaType.Parse(mapped);
                return true;
            }
        }

        if (FileNameMap.TryGetValue(name, out mediaType))
            return true;
