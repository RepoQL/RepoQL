...
public sealed class SlnLoader(ITemplateRenderer? renderer = null) : IFormatLoader, IFormatMaterializer
{
    ...
    public Records Materialize(DocumentModel document)
    {
        ...
        var headline = _renderer.Render("explore/headline-sln", model);
        var summary = _renderer.Render("explore/summary-sln", model);
        var structure = _renderer.Render("explore/structure-sln", model);
