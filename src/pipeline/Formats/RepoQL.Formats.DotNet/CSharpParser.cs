...
public sealed class CSharpParser(CSharpLoader loader, ILogger<CSharpParser>? logger = null)
    : IAsyncPipeline<IClassifiedArtifact, Records?>
{
    private readonly CSharpLoader _loader = loader ?? throw new ArgumentNullException(nameof(loader));
    private readonly ILogger<CSharpParser> _logger = logger ?? NullLogger<CSharpParser>.Instance;
        ...
        CancellationToken token)
    {
        ...
        {
            _logger.LogError(ex, "Failed to parse C# file {Uri}", item.Uri);
