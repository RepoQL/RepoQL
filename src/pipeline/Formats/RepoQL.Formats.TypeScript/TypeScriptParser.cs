    ...
    : IAsyncPipeline<IClassifiedArtifact, Records?>
{
    ...
    private readonly TypeScriptLoader _loader = loader ?? throw new ArgumentNullException(nameof(loader));
    private readonly ILogger<TypeScriptParser> _logger = logger ?? NullLogger<TypeScriptParser>.Instance;
        ...
        CancellationToken token)
    {
        ...
        try
        {
            ...
            _logger.LogDebug(
                "Parsed TypeScript/JavaScript file {Uri}: {NodeCount} nodes, {SpanCount} spans",
                item.Uri,
        ...
        {
            _logger.LogError(ex, "Failed to parse TypeScript/JavaScript file {Uri}", item.Uri);
