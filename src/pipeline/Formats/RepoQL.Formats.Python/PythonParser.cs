...
public sealed class PythonParser(PythonLoader loader, ILogger<PythonParser>? logger = null)
    : IAsyncPipeline<IClassifiedArtifact, Records?>
{
    private readonly PythonLoader _loader = loader ?? throw new ArgumentNullException(nameof(loader));
    private readonly ILogger<PythonParser> _logger = logger ?? NullLogger<PythonParser>.Instance;
    ...
    {
        if (!PythonMediaTypes.IsSupportedKind(item.MediaType?.Kind))
        {
        ...
        try
        {
            ...
            _logger.LogDebug(
                "Parsed Python file {Uri}: {NodeCount} nodes, {SpanCount} spans",
                item.Uri,
        ...
        {
            _logger.LogError(ex, "Failed to parse Python file {Uri}", item.Uri);
