...
public static class OptionValue
{
    ...
    /// </summary>
    /// <param name="intent">The agent's intent for the xray operation.</param>
    ...
    public static double GetValue(Intent intent, RepresentationLevel level)
    {
        return (intent, level) switch
        {
            ...
            (Intent.Explore, RepresentationLevel.Minimal) => 0.8,
            (Intent.Explore, RepresentationLevel.Compact) => 0.4,
            (Intent.Explore, RepresentationLevel.Standard) => 0.2,
            (Intent.Explore, RepresentationLevel.Rich) => 0.1,

            // Find: adapts to distribution. Compact and Standard are most valuable for locating things.
            (Intent.Find, RepresentationLevel.Minimal) => 0.6,
            (Intent.Find, RepresentationLevel.Compact) => 0.7,
            (Intent.Find, RepresentationLevel.Standard) => 0.5,
