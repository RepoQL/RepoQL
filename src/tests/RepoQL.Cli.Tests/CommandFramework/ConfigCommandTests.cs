...
internal sealed class ConfigCommandTests : IDisposable
{
    ...
    {
        var set = await _command.Set("llm.api_key", "abcd1234yz", "local", CancellationToken.None);
    ...
    {
        var result = await _command.Set("llm.api_key", "top-secret", "repo", CancellationToken.None);
