...
public static class DistributionAnalyzer
{
    ...
    /// </summary>
    public static DistributionAnalysis Analyze(IReadOnlyList<XrayResult> results)
    {
        if (results.Count == 0)
        {
            return new DistributionAnalysis(
                Array.Empty<XrayResult>(),
                Array.Empty<XrayResult>(),
                Array.Empty<XrayResult>(),
        ...
        // Classify into tiers
        var topTier = new List<XrayResult>();
        var middleTier = new List<XrayResult>();
        var bottomTier = new List<XrayResult>();
    ...
    private static DistributionShape DetectShape(List<XrayResult> sorted, int topTierCount)
    {
        if (sorted.Count <= 1)
    ...
    private static int GetPercentileValue(List<XrayResult> sorted, double percentile)
    {
        if (sorted.Count == 0)
            return 0;
