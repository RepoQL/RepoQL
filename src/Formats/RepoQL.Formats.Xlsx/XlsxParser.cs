...
/// <summary>
/// Pipeline processor for parsing XLSX files.
///
/// Purpose: Integrates XlsxLoader into the indexing pipeline. Converts
/// classified XLSX artifacts into graph records.
///
/// Complexity: Minimal - delegates to XlsxLoader for actual parsing.
/// </summary>
public sealed class XlsxParser(XlsxLoader loader, ILogger<XlsxParser>? logger = null)
    : IAsyncPipeline<IClassifiedArtifact, Records?>
{
    private readonly XlsxLoader _loader = loader ?? throw new ArgumentNullException(nameof(loader));
    private readonly ILogger<XlsxParser> _logger = logger ?? NullLogger<XlsxParser>.Instance;
    ...
    {
        if (item.MediaType?.Kind != "xlsx.workbook")
        {
        ...
        try
        {
            ...
            _logger.LogDebug(
                "Parsed XLSX file {Uri}: {NodeCount} nodes, {SpanCount} spans",
                item.Uri,
        ...
        {
            _logger.LogError(ex, "Failed to parse XLSX file {Uri}", item.Uri);
