...
public sealed class PHPParser(PHPLoader loader, ILogger<PHPParser>? logger = null)
    : IAsyncPipeline<IClassifiedArtifact, Records?>
{
    ...
    private readonly PHPLoader _loader = loader ?? throw new ArgumentNullException(nameof(loader));
    private readonly ILogger<PHPParser> _logger = logger ?? NullLogger<PHPParser>.Instance;
        ...
        CancellationToken token)
    {
        ...
        try
        {
            ...
            _logger.LogDebug(
                "Parsed PHP file {Uri}: {NodeCount} nodes, {SpanCount} spans",
                item.Uri,
        ...
        {
            _logger.LogError(ex, "Failed to parse PHP file {Uri}", item.Uri);
