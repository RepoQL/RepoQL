...
public sealed class GoParser(GoLoader loader, ILogger<GoParser>? logger = null)
    : IAsyncPipeline<IClassifiedArtifact, Records?>
{
    private readonly GoLoader _loader = loader ?? throw new ArgumentNullException(nameof(loader));
    private readonly ILogger<GoParser> _logger = logger ?? NullLogger<GoParser>.Instance;
    ...
    {
        if (!GoMediaTypes.IsSupportedKind(item.MediaType?.Kind))
        {
        ...
        try
        {
            ...
            _logger.LogDebug(
                "Parsed Go file {Uri}: {NodeCount} nodes, {SpanCount} spans",
                item.Uri,
        ...
        {
            _logger.LogError(ex, "Failed to parse Go file {Uri}", item.Uri);
