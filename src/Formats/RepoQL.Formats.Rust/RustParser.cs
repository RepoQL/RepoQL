...
public sealed class RustParser(RustLoader loader, ILogger<RustParser>? logger = null)
    : IAsyncPipeline<IClassifiedArtifact, Records?>
{
    private readonly RustLoader _loader = loader ?? throw new ArgumentNullException(nameof(loader));
    private readonly ILogger<RustParser> _logger = logger ?? NullLogger<RustParser>.Instance;
    ...
    {
        if (!RustMediaTypes.IsSupportedKind(item.MediaType?.Kind))
        {
        ...
        try
        {
            ...
            _logger.LogDebug(
                "Parsed Rust file {Uri}: {NodeCount} nodes, {SpanCount} spans",
                item.Uri,
        ...
        {
            _logger.LogError(ex, "Failed to parse Rust file {Uri}", item.Uri);
