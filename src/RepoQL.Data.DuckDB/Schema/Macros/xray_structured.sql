-- xray_structured(keywords, tokens, intent, scope, boost, penalize) -> TABLE
-- Returns xray results as structured rows for custom processing.
...
--
-- Example:
--   SELECT * FROM xray_structured('embeddings', tokens := 1000);
--   SELECT uri, confidence FROM xray_structured('auth', tokens := 500) WHERE depth = 0;
...
--   );
--
CREATE OR REPLACE MACRO xray_structured(
    keywords,
    tokens := 1000,
    intent := 'Find',
    scope := NULL,
