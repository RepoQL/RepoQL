...
internal sealed class ExecuteTool(QueryExecutor queryExecutor, SelfTestRunner selfTestRunner, SessionOrientation sessionOrientation)
{
    ...
    private const string ExecuteInstructions = """
        JavaScript in a WASM sandbox with the full graph at your fingertips. Query, read, render diagrams, convert documents, process media — compose capabilities that don't exist as standalone tools. The index is wild magic — your instincts are probably right, try them.
