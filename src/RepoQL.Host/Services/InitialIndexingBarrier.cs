    ...
    : BackgroundService, IInitialIndexingBarrier
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        try
        {
            // 1) Wait for initial indexing to reach an idle state
            await indexer.WaitForIdle(stoppingToken).ConfigureAwait(false);

            // 2) Refresh search projection + FTS so first queries have results
            try
            {
            ...
            // 3) Signal readiness immediately (lexical search available). Start embeddings in background.
    ...
    private async Task BackgroundEmbedAsync(CancellationToken ct)
    {
        ...
        {
            _logger.LogInformation("Background embedding refresh canceled");
        }
        catch (Exception ex)
        {
