...
/// <summary>
/// Produces rendering decisions for explore results.
/// </summary>
public static class DecisionEngine
{
        ...
        RenderingContext context)
    {
        ...
        var useMinimalForAll = context.Intent == Intent.Inventory
        ...
        foreach (var result in distribution.AllResults)
        {
            ...
            var tokens = ExploreTokenEstimator.Estimate(result, representation);
    ...
    private static int EstimatePreferredCost(DistributionAnalysis distribution, Intent intent)
    {
        ...
        {
            var level = intent == Intent.Inventory ? Representation.Compact : Representation.Rich;
            total += ExploreTokenEstimator.Estimate(result, level);
        }
        ...
        foreach (var result in distribution.MiddleTier)
        {
            ...
            {
                Intent.Inventory => Representation.Compact,
                Intent.Locate => Representation.Standard,
                Intent.Inspect => Representation.Standard,
        ...
        {
            total += ExploreTokenEstimator.Estimate(result, Representation.Compact);
        }
    ...
    private static Tier GetTier(ExploreResult result, DistributionAnalysis distribution)
    {
    ...
    private static RenderingDecision? TryDegrade(RenderingDecision decision)
    {
        ...
        var newTokens = ExploreTokenEstimator.Estimate(decision.Result, lowerLevel.Value);
