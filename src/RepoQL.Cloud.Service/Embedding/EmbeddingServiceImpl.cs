...
internal sealed class EmbeddingServiceImpl : EmbeddingService.EmbeddingServiceBase
{
    ...
    public override async Task<SubmitFeedbackResponse> SubmitFeedback(
        SubmitFeedbackRequest request,
        ServerCallContext context)
    {
        _logger.LogInformation(
            "FEEDBACK | session={SessionId} version={Version} platform={Platform} feedback={Feedback}",
            request.SessionId,
            request.Version,
            request.Platform,
            request.Feedback);

        var stored = _analytics is not null
            && await _analytics.WriteFeedbackAsync(request, context.CancellationToken);

        return new SubmitFeedbackResponse { Accepted = stored || _analytics is null };
    }
